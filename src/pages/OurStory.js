...
const OurStory = () => {
  ...
  useEffect(() => {
    ...
    const getMaxWidth = () => {
      return cards.reduce((val, card) => val + (card ? card.offsetWidth : 0), 0);
    };
    ...
    const extraSpace = isMobile ? 0.6 : 0.15;
    ...
    const scrollTrigger = ScrollTrigger.create({
      ...
      end: () => "+=" + (scrollDistance / scrollSpeed) * 2,
  ...
  }, [teamMembers]); // Re-run if teamMembers change, although milestones are separate, it's safer
  ...
  return (
    <main className="bg-[#EFE7D5] min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-8 lg:px-10 pt-12 md:pt-16 pb-16 md:pb-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {teamMembers.map((member, index) => {
            const getOverlayColor = (idx) => {
              if (idx % 3 === 0) return 'from-orange-600/80 via-orange-500/40 to-transparent';
              if (idx % 3 === 1) return 'from-yellow-500/80 via-yellow-400/40 to-transparent';
              return 'from-blue-600/80 via-blue-500/40 to-transparent';
            };
            ...
            return (
              ...
              >
                <div className="absolute inset-0 rounded-2xl border-2 border-transparent group-hover:border-white transition-all duration-300 pointer-events-none" style={{ zIndex: 10 }}></div>
                <div className="w-full h-full rounded-xl overflow-hidden relative">
                  <img
                    src={member.image || "/images/file 1.png"}
                    alt={member.name}
                    className="w-full h-full object-cover transition-all duration-300 group-hover:scale-110"
                    onError={(e) => { e.target.src = "/images/file 1.png"; }}
                  />
                  <div className="absolute bottom-0 left-0 right-0 p-4 text-white z-10 flex justify-between items-end">
                    <div>
                      ...
                      <p className="text-sm opacity-90 drop-shadow-sm">{member.role || 'Team Member'}</p>
                    </div>
      ...
      {/* Values & Milestones Sections (remain same) */}
      <section className="max-w-7xl mx-auto px-6 md:px-8 lg:px-10 py-16 md:py-20">
        <div className="mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
            {[
              { title: "Trust", img: "/images/image 16.png", color: "#FF6B35" },
              { title: "Belonging", img: "/images/image 17.png", color: "#FFD700" },
              { title: "Curiosity", img: "/images/image 18.png", color: "#4682B4" }
            ].map((v, i) => (
              <div key={i} className="bg-white rounded-3xl overflow-hidden shadow-xl h-[500px] group cursor-pointer transition-all duration-500 relative">
                <div className="p-8 flex flex-col items-center justify-center h-1/2 group-hover:h-3/4 transition-all duration-500 bg-white relative z-10">
                  <img src={v.img} alt={v.title} className="w-40 h-40 object-contain" />
                  <div className="absolute bottom-0 left-0 right-0 h-8 bg-white rounded-b-3xl"></div>
                </div>
                <div className="text-white px-8 py-8 text-center font-bold text-5xl h-1/2 group-hover:h-1/4 flex items-center justify-center rounded-t-3xl transition-all duration-500 relative z-5" style={{ fontFamily: 'MADE Avenue PERSONAL USE', backgroundColor: v.color }}>
                  {v.title}
                </div>
              </div>
            ))}
      ...
      <section className="bg-[#EFE7D5] pt-8 sm:pt-14 md:pt-20 pb-0 px-2 sm:px-4 md:px-8 overflow-x-hidden">
        <div className="max-w-7xl mx-auto overflow-x-hidden text-center mb-8 sm:mb-12 md:mb-20">
          <h2 className="text-3xl md:text-5xl lg:text-6xl font-serif font-bold text-[#0D1B2A] mb-4 tracking-tight">Milestones</h2>
          <p className="text-base md:text-lg text-[#2C2C2C] leading-relaxed max-w-2xl mx-auto">How It Unfolded, Year by Year</p>
        </div>

        <div
          ref={wrapperRef}
          className="wrapper flex flex-nowrap items-center"
          style={{ height: window.innerWidth < 768 ? '80vh' : '90vh', willChange: 'transform' }}
        >
          {[
            { id: "2020", title: "Where Curiosity Had Time", description: "Lockdown slowed everything down.\nA small town. Long days. A mind that wouldn't sit still.\nApnaaapan began as freelance work, learning by doing, one small project at a time." },
            { id: "2021", title: "Finding Our Flow", description: "Work started coming in.\nSo did confidence.\nWebsites, apps, social media, curiosity slowly turning into real skill." },
            { id: "2022", title: "When Comfort Wasn't Enough", description: "By the final year, it was clear this wasn't just a side thing anymore.\nThe thought of building something of our own stopped feeling distant.\nIt felt possible." },
            { id: "2023", title: "The Jump", description: "A stable job was left behind.\nJaipur became home.\nWith a small team and a lot of belief, Apnaaapan officially became a company." },
            { id: "2024", title: "Growing, But Staying Close", description: "Clients grew, local to national to international.\nThree cities. Many projects.\nBut the work stayed personal.\nAnd the culture stayed human." },
            { id: "2025", title: "Back to the Heart of It", description: "Eight quiet months in Pali.\nA team of 20+ people, building without noise.\nThen Jaipur again, this time from the Startup India office, stronger, clearer, steadier." },
            { id: "2026", title: "What Comes Next", description: "New backing. New systems.\nThe same intent.\nApnaaapan begins its next chapter, rooted in trust, people, and work that's built to last." }
          ].map((m, i) => (
            <div key={m.id} ref={el => cardsRef.current[i] = el} className="flex-shrink-0 w-[240px] sm:w-[320px] md:w-[380px] lg:w-[450px] mx-1 sm:mx-4">
              <div className="bg-white rounded-3xl p-4 sm:p-6 md:p-8 shadow-lg border border-gray-300 group relative overflow-hidden cursor-pointer transition-all duration-500 hover:shadow-2xl h-[420px] sm:h-[480px] md:h-[560px]">
                ...
                <div className="relative z-10 flex flex-col h-full">
                  <span className="text-orange-500 font-bold text-lg mb-6 block tracking-wider group-hover:text-white transition-colors duration-300">({m.id})</span>
                  <h3 className="text-xl md:text-3xl font-serif font-bold text-[#1a2236] mb-4 md:mb-6 group-hover:text-white transition-colors duration-300">{m.title}</h3>
                  <p className="text-[#22223b] leading-relaxed text-sm md:text-base group-hover:text-white transition-colors duration-300" style={{ whiteSpace: 'pre-line' }}>{m.description}</p>
                </div>
              </div>
            </div>
          ))}
