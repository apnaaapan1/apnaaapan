...
export default function AdminVideos() {
  ...
  const handleVideoUpload = async (e) => {
    ...
    const finalTitle = derivedTitle || fallbackTitle || 'Untitled Video';
    ...
    try {
      ...
      const { upload_url, api_key, timestamp, signature, folder, resource_type, public_id } = sigData;
      ...
      const uploadResult = await new Promise((resolve, reject) => {
        ...
        xhr.onload = () => {
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            reject(new Error(`Cloudinary upload failed (${xhr.status})${extra}`));
          }
        };
              ...
              <div className="flex flex-col gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    Uploads directly to Cloudinary. Large files are supported.
                  </p>
                </div>
